// Code Reference https://github.com/Rob--W/cors-anywhere/blob/master/lib/cors-anywhere.js
...
const showUsage = (help_file, headers, response) => {
  ...
  if (!bufferedFile) {
    const data = helpFile.split(",")[1];
    bufferedFile = Buffer.from(data, "base64").toString("utf-8");
    bufferedFile = bufferedFile.replace(
      "<<unknown>>",
      process.env.VERCEL_REGION || "Unknown"
    );
  }
  response.end(bufferedFile);
};
...
const isValidHostName = (hostname) =>
  !!(regexp_tld.test(hostname) || net.isIPv4(hostname) || net.isIPv6(hostname));
...
const getHandler = (options, proxy) => {
  ...
  const hasRequiredHeaders = (headers) => {
    return (
      ...
      corsAnywhere.requireHeader.some((headerName) =>
        Object.hasOwnProperty.call(headers, headerName)
      )
    );
  };
  ...
  return (req, res) => {
    ...
    // Edit: Handling encodeURIComponent encoded url
    const location = parseURL(decodeURIComponent(req.url.slice(1)));
    if (
...
export const createServer = (options) => {
  ...
  if (options.httpProxyOptions) {
    Object.keys(options.httpProxyOptions).forEach(
      (option) => (httpProxyOptions[option] = options.httpProxyOptions[option])
    );
  }
  ...
  if (options.httpsOptions) {
    server = httpsServer(options.httpsOptions, requestHandler);
  } else {
